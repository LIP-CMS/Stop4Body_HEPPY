...
class ttHCoreEventAnalyzer( Analyzer ):
    ...
    def process(self, event):
        ...
        objects20j = [ j for j in event.cleanJets if j.pt() > 20 ]
        ...
        event.mhtJet20vec = ROOT.reco.Particle.LorentzVector(-1.*(sum([x.px() for x in objects20])) , -1.*(sum([x.py() for x in objects20])), 0, 0 )
        event.mhtPhiJet20 = event.mhtJet20vec.phi()
        event.mhtJet20 = event.mhtJet20vec.pt()
        ...
        event.htJet20j = sum([x.pt() for x in objects20j])
        ...
        objects20a  = [ j for j in event.cleanJetsAll if j.pt() > 20 ] + event.selectedLeptons
        ...
        event.htJet20a = sum([x.pt() for x in objects20a])
        event.mhtJet20veca = ROOT.reco.Particle.LorentzVector(-1.*(sum([x.px() for x in objects20a])) , -1.*(sum([x.py() for x in objects20a])), 0, 0 )
        event.mhtPhiJet20a = event.mhtJet20veca.phi()
        event.mhtJet20a = event.mhtJet20veca.pt()
